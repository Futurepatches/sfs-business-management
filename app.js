...
async function login(){const user=$('loginUser').value.trim(),pass=$('loginPass').value;let url=localStorage.sfsApiUrl;if(!url){$('loginError').textContent='Enter the Apps Script Web App URL first.';return}S.api=url;const r=await api('login',{username:user,password:pass});if(!r.ok){$('loginError').textContent=r.error||'Invalid username or password';return}S.session=r.session;S.user=r.user;sessionStorage.sfsSession=S.session;enter();}
...
const pages={
    ...
    settings:()=>`<div class="wrap"><div class="panel"><h3>System Settings</h3><p class="muted">The original live inventory remains read-only. This software writes only to the separate database.</p><label>Apps Script Web App URL<input id="apiurl" class="input" value="${esc(S.api)}"></label><div class="actions"><button class="btn primary" onclick="saveSettings()">Save Settings</button></div><hr><button class="btn" onclick="changePasswordForm()">Change My Password</button>${S.user?.role==='ADMIN'?'<button class="btn" onclick="refreshSource()">Refresh Source Snapshot</button>':''}</div></div>`,
...
function renderIvLines(){if(!$('ivlines'))return;$('ivlines').innerHTML=ivl.map((x,i)=>`<tr><td><input value="${esc(x.model)}" list="ivml" onchange="ivl[${i}].model=this.value;renderIvLines()"></td><td>${esc((S.products.find(p=>p['Model / Part No.']===x.model)||{}).Description||'')}</td><td><input type="number" value="${x.qty}" onchange="ivl[${i}].qty=this.value;renderIvLines()"></td><td><input type="number" value="${x.rate}" onchange="ivl[${i}].rate=this.value;renderIvLines()"></td><td>${((+x.qty||0)*(+x.rate||0)).toFixed(2)}</td><td><button class="btn small" onclick="ivl.splice(${i},1);renderIvLines()">×</button></td></tr>`).join('');$('ivlines').insertAdjacentHTML('afterend',`<datalist id="ivml">${S.products.map(p=>`<option value="${esc(p['Model / Part No.'])}">`).join('')}</datalist><datalist id="dclist">${Array.from(new Set(S.tx.filter(t=>t.Type==='Delivery Challan').map(t=>t.Reference))).map(ref=>`<option value="${ref}">`).join('')}</datalist>`);$('ivtotal').textContent=ivl.reduce((a,x)=>a+(+x.qty||0)*(+x.rate||0),0).toFixed(2)}

// Auto-fetch items and details when DC number is selected in Invoice
function fetchDcData(dcNo){
    if(!dcNo)return;
    // Find transactions matching this DC reference
    const matches=S.tx.filter(t=>t.Type==='Delivery Challan' && t.Reference===dcNo);
    if(matches.length>0){
        // Take first match for header details
        const first=matches[0];
        $('ivcust').value=first.Party||'';
        $('ivdate').value=first.Date||$('ivdate').value;
        // Map items from transactions
        ivl=matches.map(m=>({model:m['Model / Part No.'], qty:m['Qty OUT']||1, rate:''}));
        renderIvLines();
    }
}

async function saveInvoice(print){const p={no:$('ivno').value,date:$('ivdate').value,customer:$('ivcust').value,customerId:$('ivid').value,po:$('ivpo').value,poDate:$('ivpodate').value,dc:$('ivdc').value,dcDate:$('ivdcdate').value,stn:$('ivstn').value,ntn:$('ivntn').value,address:$('ivaddr').value,attn:$('ivattn').value,items:ivl,total:$('ivtotal').textContent};const r=await api('saveInvoice',p);if(!r.ok)return alert(r.error);if(print)printInvoice(p);alert('Invoice saved.');await refresh()}
function renderCustomers(){$('customers').innerHTML=S.customers.map(c=>`<div class="kv"><b>${esc(c.Name)}</b> — ${esc(c['Customer ID'])}<br>${esc(c['Contact Person'])} • ${esc(c.Phone)} • ${esc(c.Email)}<br>${esc(c.Address)}<br><button class="btn small" onclick="ledger('Customer','${encodeURIComponent(c.Name)}')">Ledger</button></div>`).join('')||'No customers yet.'}
function renderSuppliers(){$('suppliers').innerHTML=S.suppliers.map(c=>`<div class="kv"><b>${esc(c.Name)}</b> — ${esc(c['Supplier ID'])}<br>${esc(c['Contact Person'])} • ${esc(c.Phone)} • ${esc(c.Email)}<br>${esc(c.Address)}<br><button class="btn small" onclick="ledger('Supplier','${encodeURIComponent(c.Name)}')">Ledger</button></div>`).join('')||'No suppliers yet.'}
...
function printDC(d){
    ...
    printDoc(`
        <div class="sfs-header">
            <div class="sfs-company-info"><h2>STANDARD FLUID SYSTEMS</h2><p>Pneumatic Solution Provider</p></div>
        ...
        <table class="sfs-table">
            <thead>
                <tr><th style="width: 8%;">Item</th><th style="width: 42%;">Description</th><th style="width: 22%;">Item Code #</th><th style="width: 10%;">Qty.</th><th style="width: 18%;">Rate / Unit</th></tr>
            </thead>
            <tbody>${rows}</tbody>
        </table>
...
function printInvoice(d){
    ...
    printDoc(`
        ...
        <table class="sfs-table">
            <thead>
                <tr><th style="width: 8%;">Item</th><th style="width: 38%;">Description</th><th style="width: 18%;">Item Code #</th><th style="width: 10%;">Qty.</th><th style="width: 13%;">Rate / Unit</th><th style="width: 13%;">Amount</th></tr>
            </thead>
            <tbody>${rows}</tbody>
        </table>
...
function printDoc(body){
    $('printArea').innerHTML=`<div class="sfs-doc-container">${body}</div>`;
